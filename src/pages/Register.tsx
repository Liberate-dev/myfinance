...
export const Register = () => {
  ...
  const { register, isLoading, clearError, error: storeError } = useAuth();
    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-terminal-black relative overflow-hidden">
      ...
      <div className="w-full max-w-md relative animate-fade-in">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-text-muted font-mono text-sm mt-2">Initialize your session</p>
        </div>
        ...
        <div className="card border-finance-green/20">
          <form onSubmit={handleSubmit} className="space-y-5">
              ...
              {password && (
                <div className="flex gap-3 mt-2">
                  {passwordRequirements.map((req, index) => (
                    <div
                      key={index}
                      className={`flex items-center gap-1 text-xs font-mono ${req.met ? 'text-finance-green' : 'text-text-muted'}`}
                    >
                      <span>{req.met ? <CheckIcon /> : '○'}</span>
                      {req.text}
                    </div>
                  ))}
                </div>
            ...
            {/* Confirm Password */}
            <div className="space-y-2">
              <label htmlFor="confirm-password" className="block text-xs font-mono uppercase tracking-wider text-text-muted">
                Confirm Password
              </label>
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-text-muted">
                ...
                <input
                  ...
                  className={`input pl-12 ${confirmPassword && password !== confirmPassword ? 'border-finance-red' : ''}`}
                  placeholder="Confirm password"
                  disabled={isLoading}
                />
              </div>
              {confirmPassword && password !== confirmPassword && (
                <p className="text-xs font-mono text-finance-red mt-1">PASSWORD MISMATCH</p>
              )}
            </div>
          ...
          <div className="mt-6 pt-6 border-t border-terminal-border text-center">
            <p className="text-text-muted text-sm font-mono">
              HAVE ACCOUNT?{' '}
              <Link to="/login" className="text-finance-green hover:text-finance-green-dim font-semibold">
                SIGN IN
              </Link>
            </p>
          </div>
